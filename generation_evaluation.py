...
def my_sample(model, gen_data_dir, sample_batch_size = 50, obs = (3,32,32), sample_op = sample_op):
...
if __name__ == "__main__":
    ...
    model = PixelCNN(nr_resnet=2, nr_filters=20, input_channels=3, nr_logistic_mix=10)
    model = model.to(device)
    model = model.eval()
    model.load_state_dict(torch.load('models/pcnn_cpen455_from_scratch_274.pth'))
